...
public class KernalDistance {
		private Segment segment1;
		private Segment segment2;

		public KernalDistance(Segment s1, Segment s2) {
			this.segment1 = s1;
			this.segment2 = s2;
		}
		...
		public double horizonDist() {
			...
			if (abDist <= cdDist) {
				...
				double hDis = 1 / (1 + Math.exp(-1/2*Math.max(h1, h2) + 5));
				return hDis;
			} else {
		...
		public double totalDist() {
			double totalDistance = this.perpendDist()*this.perpendDist()+this.horizonDist()*this.horizonDist()+this.dirDist()*this.dirDist()+
					this.velocity()*this.velocity()+this.durDist()*this.durDist()+this.timeDist()*this.timeDist();
			return Double.parseDouble(String.format("%.3f", 1-Math.exp(-(totalDistance/2.0))));
		}
	...
	public static void main(String[] args) {
		...
		KernalDistance sd = new KernalDistance(segment1, segment2);
